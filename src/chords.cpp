...
struct Chords : Module
{
	...
	Chords()
	{
        ...
        configParam(PROG_PARAM, 1.0f, 10.0f, 1.0f);    //, 0.0f, 12.0f, 12.0f));
	}
	...
	void process(const ProcessArgs &args) override
	{
        ...
        patchNum = clamp((params[PROG_PARAM].getValue() + inputs[PROG_INPUT].getVoltage()),1.f,10.f);
		fileDesc = "";
		fileDesc = std::to_string(patchNum);
		
		prg_prm.step(patchNum / 10.0f);
		act_prm = true;
		...
		if (act_prm)
		{
			...
			for (int j = 0; j < T; ++j)
			{
				if (note_trigger[j].process(params[NOTE_PARAM +j].getValue()))
				{
